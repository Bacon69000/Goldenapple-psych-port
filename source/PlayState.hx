...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'applecore':
				...
				scaryBG = new FlxSprite(-350, -375).loadGraphic(Paths.image('goldenApple/yeah'));
				...
				swagBG = new FlxSprite(-600, -200).loadGraphic(Paths.image('goldenApple/hi'));
				...
				unswagBG = new FlxSprite(-600, -200).loadGraphic(Paths.image('goldenApple/poop'));
				...
				for (i in 0...2) {
					...
					pizza.frames = Paths.getSparrowAtlas('goldenApple/pizza');
			...
			case 'disruption':
				defaultCamZoom = 0.9;
				gfSpeed = 2;
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('goldenApple_dave/disruptor'));
				bg.active = true;
				add(bg);
			...
			case 'disability':
				defaultCamZoom = 0.9;
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('goldenApple_dave/disabled'));
				bg.active = true;
				add(bg);
				var testshader:Shaders.GlitchEffect = new Shaders.GlitchEffect();
				testshader.waveAmplitude = 0.1;
			...
			case 'algebra':
				...
				defaultCamZoom = 0.85;
				swagSpeed = 1.6;
				var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('goldenApple_dave/algebraBg'));
				...
				davePiss.frames = Paths.getSparrowAtlas('goldenApple_dave/bgJunkers/davePiss');
		...
		if(SONG.player2 == 'bandu-candy' || SONG.player2 == 'bambi-piss-3d')
		{
			dadDanceSnap = 1;
		}
	
		if(SONG.player1 == 'bandu-candy' || SONG.player1 == 'bambi-piss-3d')
		{
			danceBeatSnap = 1;
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			default:
				dadmirror = new Character(100, 100, dad.curCharacter);
		}
		dadmirror.y += 0;
		dadmirror.x += 150;
		...
		switch (curSong.toLowerCase())
		{
			...
			case 'insanity':
				preload('goldenApple_dave/redsky');
				preload('goldenApple_dave/redsky_insanity');
			case 'wireframe':
				preload('goldenApple/badai');
			case 'algebra':
				preload('goldenApple_dave/HALL_MONITOR');
				preload('goldenApple_dave/diamondMan');
				preload('goldenApple_dave/playrobot');
				preload('goldenApple_dave/ohshit');
				preload('goldenApple_dave/garrett_algebra');
				preload('goldenApple_dave/og_dave_angey');
			case 'recovered-project':
				preload('goldenApple_dave/recovered_project_2');
				preload('goldenApple_dave/recovered_project_3');
		}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, gottaHitNote, daNoteStyle);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, true, gottaHitNote);
		...
		if (altSong != null) {
			...
			altUnspawnNotes.sort(sortByTime);
		}
	}
	...
	public static var wtfThing:Bool = false;
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					{
						opponentStrums.forEach(function(sprite:Strum)
							{
								if (Math.abs(Math.round(Math.abs(daNote.noteData)) % 4) == sprite.ID)
								{
									sprite.animation.play('confirm', true);
				...
				if (((daNote.y < -daNote.height && !ClientPrefs.downScroll || daNote.y >= strumliney + 106 && ClientPrefs.downScroll) && SONG.song.toLowerCase() != 'applecore') 
					|| (SONG.song.toLowerCase() == 'applecore' && unfairPart && daNote.y >= strumliney + 106) 
					|| (SONG.song.toLowerCase() == 'applecore' && !unfairPart && (daNote.y < -daNote.height && !ClientPrefs.downScroll || daNote.y >= strumliney + 106 && ClientPrefs.downScroll)))
				{
					/*
				...
				if (SONG.song.toLowerCase() == 'applecore' && unfairPart == true)
				{
	...
	function swapDad(char:String, x:Float = 100, y:Float = 100, flash:Bool = true)
	{
		...
		add(dad);
		trace('added dad');*/
		var ndad:Character = new Character(x, y, char, false);
		dadMap.set(char, ndad);
		dadGroup.add(ndad);
		trace('set dad');
		trace('repositioned dad');
		startCharacterPos(ndad, true);
		ndad.alpha = 0.00001;
		startCharacterLua(ndad.curCharacter);
		trace('added dad');
		if(flash)
	...
	function bgImg(Path:String) {
		return Paths.image('goldenApple_dave/bgJunkers/$Path');
	}
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			...
			var spr:Strum = playerStrums.members[key];
			...
			case 'algebra':
				switch(curBeat)
				{
					...
					case 696:
						...
						algebraStander('garrett', garrettStand, 500, 225, true);
						algebraStander('og-dave-angey', daveStand, 250, 100);
						iconP2.changeIcon('hall-monitor');
					case 1344:
						//DIAMOND MAN TURN
						swapDad('diamond-man');
						...
						algebraStander('hall-monitor', hallMonitorStand, 0, 100);
						algebraStander('og-dave-angey', daveStand, 250, 100);
						iconP2.changeIcon('diamond');
					case 1696:
						//PLAYROBOT TURN
						swapDad('playrobot');
						swagSpeed = 1.6;
						iconP2.changeIcon('playrobot');
					case 1852:
			...
			case 'applecore':
				switch(curBeat) {
					...
					case 223:
						wtfThing = true;
						what.forEach(function(spr:FlxSprite){
							spr.frames = Paths.getSparrowAtlas('goldenApple/minion');
					...
					case 636:
						...
						gfSpeed = 1;
						wtfThing = false;
						var dumbStupid = new FlxSprite().loadGraphic(Paths.image('goldenApple/poop'));
					...
					case 667:
						FlxTween.tween(littleIdiot, {"scale.x": littleIdiot.scale.x + 2.1, "scale.y": littleIdiot.scale.y + 2.1}, 1.35, {ease: FlxEase.cubeInOut, onComplete: function(twn:FlxTween){
							iconP2.changeIcon("unfair");
							orbit = false;
							dad.visible = dadmirror.visible = swagger.visible = false;
							var derez = new FlxSprite(dad.getMidpoint().x, dad.getMidpoint().y).loadGraphic(Paths.image('goldenApple/monkey_guy'));
							derez.setPosition(derez.x - derez.width / 2, derez.y - derez.height / 2);
							derez.antialiasing = false;
							add(derez);
							var deez = new FlxSprite(swagger.getMidpoint().x, swagger.getMidpoint().y).loadGraphic(Paths.image('goldenApple/monkey_person'));
